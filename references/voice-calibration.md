...
Use the lookup in SKILL.md for reading. For new personal/global setup, write `$CODEX_HOME/personalizer/voice-profile.md` when `CODEX_HOME` is set, otherwise `~/.codex/personalizer/voice-profile.md`. For project-specific author or brand setup, use `<project-root>/.personalizer/voice-profile.md`. Follow an explicitly chosen safe profile destination outside the installed skill. Do not turn an unrelated file named `voice-profile.md` into this schema.
...
The helper performs local computation and makes no network calls. Codex still processes supplied writing in its normal model session; do not claim the whole workflow is offline. Do not upload samples to another analysis service.
